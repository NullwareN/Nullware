...
bool CAimbotGlobal::PlayerBoneInFOV(CTFPlayer *pTarget, Vec3 vLocalPos,
  ...
  // Quick center-based FOV check to avoid expensive bone calculations for targets clearly outside FOV
  {
    Vec3 vCenter = pTarget->GetCenter();
    Vec3 vCenterAngle = Math::CalcAngle(vLocalPos, vCenter);
    float flCenterFOV = Math::CalcFov(vLocalAngles, vCenterAngle);
    if (flCenterFOV > Vars::Aimbot::General::AimFOV.Value + 40.f)
      return false;
  }
  ...
  const int nHitboxes = pTarget->GetNumOfHitboxes();
  for (int nHitbox = 0; nHitbox < nHitboxes; nHitbox++) {
    ...
    // Optimization: if we found a point very close to center, we can probably stop looking
    if (flMinFOV < 1.0f)
      break;
  }
