...
bool CFakeLag::IsAllowed(CTFPlayer *pLocal, CTFWeaponBase *pWeapon,
                         CUserCmd *pCmd) {
  ...
  if (!(Vars::Fakelag::Fakelag.Value || m_bUnducking) ||
      ...
      F::Ticks.m_iShiftedGoal != F::Ticks.m_iShiftedTicks ||
      F::Ticks.m_bRecharge)
    return false;

  if (G::Attacking == 1 && Vars::Fakelag::UnchokeOnAttack.Value ||
      ...
      pWeapon &&
          ...
          !(G::LastUserCmd->buttons & IN_ATTACK)) // try to prevent issues
    return false;

  if (m_bUnducking)
    return true;

  const bool bMoving = !(Vars::Fakelag::Options.Value &
                         Vars::Fakelag::OptionsEnum::OnlyMoving) ||
                       pLocal->m_vecVelocity().Length2D() > 10.f;
  if (!bMoving)
    return false;
  ...
  case Vars::Fakelag::FakelagEnum::Adaptive: {
    static auto sv_lagcompensation_teleport_dist =
        H::ConVars.FindVar("sv_lagcompensation_teleport_dist");
    const float flDist =
        powf(sv_lagcompensation_teleport_dist->GetFloat(), 2.f);
...
void CFakeLag::PreserveBlastJump(CTFPlayer *pLocal) {
  ...
  if (!Vars::Fakelag::RetainBlastJump.Value ||
      Vars::Misc::Movement::AutoRocketJump.Value ||
      Vars::Misc::Movement::AutoCTap.Value || !pLocal->IsAlive() ||
      pLocal->IsAGhost() ||
      Vars::Fakelag::RetainSoldierOnly.Value &&
          pLocal->m_iClass() != TF_CLASS_SOLDIER)
    return;
...
void CFakeLag::Unduck(CTFPlayer *pLocal, CUserCmd *pCmd) {
  ...
  if (!(pLocal->m_hGroundEntity() && pLocal->IsDucking() &&
...
void CFakeLag::Run(CTFPlayer *pLocal, CTFWeaponBase *pWeapon, CUserCmd *pCmd,
  ...
  if (!m_iGoal) {
    switch (Vars::Fakelag::Fakelag.Value) {
    case Vars::Fakelag::FakelagEnum::Off:
      m_iGoal = 0;
      break;
    case Vars::Fakelag::FakelagEnum::Plain:
      m_iGoal = Vars::Fakelag::PlainTicks.Value;
      break;
    case Vars::Fakelag::FakelagEnum::Random:
      m_iGoal = SDK::StdRandomInt(Vars::Fakelag::RandomTicks.Value.Min,
                                  Vars::Fakelag::RandomTicks.Value.Max);
      break;
    case Vars::Fakelag::FakelagEnum::Adaptive:
      m_iGoal = 22;
      break;
