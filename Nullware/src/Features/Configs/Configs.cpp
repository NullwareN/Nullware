...
CConfigs::CConfigs()
{
	...
	size_t size = 0;
	if (_dupenv_s(&localAppData, &size, "LOCALAPPDATA") == 0 && localAppData != nullptr)
	{
		m_sConfigPath = std::string(localAppData) + "\\Nullware\\";
		free(localAppData);
	}
	else
...
static inline void LoadMain(BaseVar*& pBase, boost::property_tree::ptree& tTree)
{
	...
	if (auto tMap = tTree.get_child_optional(pVar->Name()))
	{
		...
		SDK::Output("Nullware", std::format("{} not found", pVar->Name()).c_str(), ALTERNATE_COLOR, OUTPUT_CONSOLE | OUTPUT_MENU | OUTPUT_DEBUG);
}
...
bool CConfigs::SaveConfig(const std::string& sConfigName, bool bNotify)
{
	try
	{
		...
		if (bNotify)
			SDK::Output("Nullware", std::format("Config {} saved", sConfigName).c_str(), DEFAULT_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
	}
	catch (...)
	{
		SDK::Output("Nullware", "Save config failed", ALTERNATE_COLOR, OUTPUT_CONSOLE | OUTPUT_MENU | OUTPUT_DEBUG);
		return false;
	}
...
bool CConfigs::LoadConfig(const std::string& sConfigName, bool bNotify)
{
	try
	{
		...
		if (auto tSub = tRead.get_child_optional("Binds"))
		{
			...
			SDK::Output("Nullware", "Config binds not found", ALTERNATE_COLOR, OUTPUT_CONSOLE | OUTPUT_MENU | OUTPUT_DEBUG);
			...
			tSub || (tSub = tRead.get_child_optional("ConVars")))
		{
			...
			SDK::Output("Nullware", "Config vars not found", ALTERNATE_COLOR, OUTPUT_CONSOLE | OUTPUT_MENU | OUTPUT_DEBUG);
		...
		if (auto tSub = tRead.get_child_optional("Groups"))
		{
			...
			SDK::Output("Nullware", "Config groups not found", ALTERNATE_COLOR, OUTPUT_CONSOLE | OUTPUT_MENU | OUTPUT_DEBUG);
		...
		m_sCurrentConfig = sConfigName; m_sCurrentVisuals = "";
		if (bNotify)
			SDK::Output("Nullware", std::format("Config {} loaded", sConfigName).c_str(), DEFAULT_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
	}
	catch (...)
	{
		SDK::Output("Nullware", "Load config failed", ALTERNATE_COLOR, OUTPUT_CONSOLE | OUTPUT_MENU | OUTPUT_DEBUG);
		return false;
	}
...
bool CConfigs::SaveVisual(const std::string& sConfigName, bool bNotify)
{
	try
	{
		...
		if (bNotify)
			SDK::Output("Nullware", std::format("Visual config {} saved", sConfigName).c_str(), DEFAULT_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
	}
	catch (...)
	{
		SDK::Output("Nullware", "Save visuals failed", ALTERNATE_COLOR, OUTPUT_CONSOLE | OUTPUT_MENU | OUTPUT_DEBUG);
		return false;
	}
	return true;
}
...
bool CConfigs::LoadVisual(const std::string& sConfigName, bool bNotify)
{
	try
	{
		...
		if (bNotify)
			SDK::Output("Nullware", std::format("Visual config {} loaded", sConfigName).c_str(), DEFAULT_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
	}
	catch (...)
	{
		SDK::Output("Nullware", "Load visuals failed", ALTERNATE_COLOR, OUTPUT_CONSOLE | OUTPUT_MENU | OUTPUT_DEBUG);
		return false;
	}
	return true;
}
...
void CConfigs::DeleteConfig(const std::string& sConfigName, bool bNotify)
{
	try
	{
		...
		if (bNotify)
			SDK::Output("Nullware", std::format("Config {} deleted", sConfigName).c_str(), DEFAULT_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
	}
	catch (...)
	{
		SDK::Output("Nullware", "Remove config failed", ALTERNATE_COLOR, OUTPUT_CONSOLE | OUTPUT_MENU | OUTPUT_DEBUG);
	}
}
...
void CConfigs::ResetConfig(const std::string& sConfigName, bool bNotify)
{
	try
	{
		...
		if (bNotify)
			SDK::Output("Nullware", std::format("Config {} reset", sConfigName).c_str(), DEFAULT_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
	}
	catch (...)
	{
		SDK::Output("Nullware", "Reset config failed", ALTERNATE_COLOR, OUTPUT_CONSOLE | OUTPUT_MENU | OUTPUT_DEBUG);
	}
}
...
void CConfigs::DeleteVisual(const std::string& sConfigName, bool bNotify)
{
	try
	{
		...
		if (bNotify)
			SDK::Output("Nullware", std::format("Visual config {} deleted", sConfigName).c_str(), DEFAULT_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
	}
	catch (...)
	{
		SDK::Output("Nullware", "Remove visuals failed", ALTERNATE_COLOR, OUTPUT_CONSOLE | OUTPUT_MENU | OUTPUT_DEBUG);
	}
}
...
void CConfigs::ResetVisual(const std::string& sConfigName, bool bNotify)
{
	try
	{
		...
		if (bNotify)
			SDK::Output("Nullware", std::format("Visual config {} reset", sConfigName).c_str(), DEFAULT_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
	}
	catch (...)
	{
		SDK::Output("Nullware", "Reset visuals failed", ALTERNATE_COLOR, OUTPUT_CONSOLE | OUTPUT_MENU | OUTPUT_DEBUG);
	}
}