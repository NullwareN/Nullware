...
void CPlayerlistCore::SavePlayerlist() {
  ...
  try {
    ...
    SDK::Output("Nullware", "Saved playerlist", DEFAULT_COLOR,
                OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
  } catch (...) {
    SDK::Output("Nullware", "Save playerlist failed", ALTERNATE_COLOR,
...
void CPlayerlistCore::LoadPlayerlist() {
  ...
  try {
    ...
    if (auto tSub = tRead.get_child_optional("Config")) {
      ...
      SDK::Output("Nullware", "Playerlist config not found", ALTERNATE_COLOR,
                  OUTPUT_CONSOLE | OUTPUT_MENU | OUTPUT_DEBUG);
    ...
    if (auto tSub = tRead.get_child_optional("Tags")) {
      ...
      SDK::Output("Nullware", "Playerlist tags not found", ALTERNATE_COLOR,
                  OUTPUT_CONSOLE | OUTPUT_MENU | OUTPUT_DEBUG);
    ...
    if (auto tSub = tRead.get_child_optional("Aliases")) {
      ...
      SDK::Output("Nullware", "Playerlist aliases not found", ALTERNATE_COLOR,
                  OUTPUT_CONSOLE | OUTPUT_MENU | OUTPUT_DEBUG);

    F::PlayerUtils.m_bLoad = false;
    SDK::Output("Nullware", "Loaded playerlist", DEFAULT_COLOR,
                OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
  } catch (...) {
    SDK::Output("Nullware", "Load playerlist failed", ALTERNATE_COLOR,
                OUTPUT_CONSOLE | OUTPUT_MENU | OUTPUT_DEBUG);
  }
}
