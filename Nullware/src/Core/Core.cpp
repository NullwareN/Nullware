...
static inline bool CheckDXLevel()
{
	...
	{
		/*
		const char* sMessage = "You are running with graphics options that Nullware does not support. -dxlevel must be at least 90.";
		U::Core.AppendFailText(sMessage);
		SDK::Output("Nullware", sMessage, DEFAULT_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
		return false;
		*/

		const char* sMessage = "You are running with graphics options that Nullware does not support. It is recommended for -dxlevel to be at least 90.";
		SDK::Output("Nullware", sMessage, DEFAULT_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
	}
...
void CCore::LogFailText()
{
	try
	{
		...
		m_ssFailStream << "Logged to Nullware\\fail_log.txt. ";
	}
	catch (...) {}
...
void CCore::Load()
{
	...
	SDK::Output("Nullware", "Loaded", DEFAULT_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
}
...
void CCore::Unload()
{
	...
	SDK::Output("Nullware", "Unloaded", DEFAULT_COLOR, OUTPUT_CONSOLE | OUTPUT_DEBUG);
}